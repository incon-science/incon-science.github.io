...
func update_shape_scale() -> void:
	...
	sprite.scale = sprite.scale.lerp(_default_sprite_scale * sprite.scale.sign(), shape_rescale_weight)
...
func apply_squash() -> void:
	...
	sprite.scale.x *= remap(velocity.y, 0.0, max_fall_speed, squash_width_scale_at_rest, squash_width_scale_at_max_fall)
	sprite.scale.y *= remap(velocity.y, 0.0, max_fall_speed, squash_height_scale_at_max_fall, squash_height_scale_at_rest)
...
func sprite_animation() -> void:
	Engine.time_scale = 1
	#pour les ANIMS up jump et le wall jump VOIR PLUS HAUT dans les fonctions eponymes
	if is_on_floor() :
		if velocity.x == 0.0 :
