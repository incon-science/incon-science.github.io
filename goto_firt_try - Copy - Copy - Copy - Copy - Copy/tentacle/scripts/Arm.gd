...
func _physics_process(delta: float) -> void:
	...
	if player != null :
		var decalage = position-player.position+Vector2(256,256)
		var decalage_reverse = decalage*Vector2(-1,-1)
		target_pos = decalage_reverse+Vector2(256,256)
	
	if _direction_target != null:
		var position_edge_tentacle = _segments[_segments.size()-1]
		_direction_target_in_progress += _direction_target*0.001*delta
		#pas sur du > sur deux vecteurs qui ont pas forcement la meme direction ? 
		solve_ik(position_edge_tentacle+_direction_target_in_progress)
	else :
		solve_ik(target_pos)
...
func slowly_add_direction_target(direction_target):
	_direction_target = direction_target
	_direction_target_in_progress = Vector2(0,0)
